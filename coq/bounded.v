...
Section Bounded.
  Variables (aT rT : Type) (x0 : rT).
  Variables (measure : aT -> nat) (F : (aT -> rT) -> (aT -> rT)).
  
  Hypothesis F_wf : 
    forall f g x, (forall y, measure y < measure x -> f y = g y) -> F f x = F g x.
  ...
  Lemma F_rec_narrow m n x : 
    measure x < n -> n <= m -> F_rec m x = F_rec n x.
  Proof.
    elim: n m x => // => n IHn [//|m] x A B /=. 
    apply: F_wf => y Hy. apply: IHn => //. exact: leq_trans Hy _. 
  Qed.

  Definition Fix x := F_rec (measure x).+1 x.
  
  Lemma Fix_eq x : Fix x = F Fix x.
  Proof. 
    rewrite /Fix /=. apply: F_wf => y Hy.
